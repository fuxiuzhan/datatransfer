...
public class DataTransferLocalServer extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            b.group(group).channel(NioSocketChannel.class).option(ChannelOption.ALLOCATOR, UnpooledByteBufAllocator.DEFAULT).option(ChannelOption.TCP_NODELAY, true).option(ChannelOption.SO_LINGER, 0).handler(new ChannelInitializer<SocketChannel>() {
                @Override
                                ...
                                switch (((BaseMessage) msg).getMessageType()) {
                                    ...
                                    case Const.MSG_CONNECT:
                                        try {
                                            ...
                                            connector.start();
                                            LocalServerConfig.getConnectMap().put(connectMessage.getlSocketId(), connector);
                                        } catch (Exception e) {
